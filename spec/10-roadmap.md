...
- ⏳ **4F — Sealed-block selection + copy.** Split into:
    ...
    - ⏳ **4F-cross-block — cross-block selection + copy.** Drag across boundaries; copy concatenates block text, skipping chrome. Needs the pane-level `PaneSelection { anchor, head: PaneCursor }` per [04 §"Cross-block selection"](04-prompt-editor.md#cross-block-selection) and the per-row highlight clipping that talks across multiple painted blocks. Deferred behind sticky-top headers (4E) since those affect what "above viewport" means during a long drag.
- ⏳ **4G — Block header chrome.** Split into:
    ...
    - ⏳ **4G-async-context — git branch + dirty + live duration.** Async-probe surface per spec/00 §"Do not block the UI on probes"; co-locates with Phase 5's `termica-context` work since the same probes power both surfaces. Live duration timer for `Running` blocks lands here too — it needs the wall-clock plumbing that the frame-counter-as-duration placeholder still doesn't have.
