...
- **zsh / bash** ([`completion_request_bytes_sentinel`](../src/submit_framing.rs)): a guarded command `__termica_complete <id> <base64(line)>` + `\r`. Neither has a read-eval loop — both run interactively (zsh with `unsetopt zle`, bash with `--noediting`) and *execute* each input line — so the request is dispatched as a real command. The bootstrap's `termica_preexec` / `termica_precmd` recognise the `__termica_complete` sentinel and emit **nothing** for it (inert), and the function preserves the user's last `$?`. The two differ in **one** detail — how the sentinel is kept out of history:
  - **zsh** prepends a space and relies on a `zshaddhistory` hook (and `hist_ignore_space`).
  - **bash** must **not** prepend a space: bash-preexec reads the running command from `builtin history 1`, and a leading-space command vanishes under `HISTCONTROL=ignorespace`, which would make bash-preexec report the *previous* command and corrupt the block model. So the bash sentinel enters history normally (where the guards see it) and `__termica_complete` **deletes its own history entry**.
...
**Reply (shell → Termica):** `{"type":"completion","id":<n>,"filenames":<bool>,"cur":<word>,"value":[<line>, …]}` — a JSON array of the shell's **raw** completion lines (fish: `complete -C` output, `value\tdescription` or a space-padded table; zsh/bash: value-only lines). The split into value/description is done once, in Rust ([`parse_shell_complete`](../src/completion/drivers/parse.rs)), so the shells can't diverge. fish runs `complete -C <line>` **in-process** and loops straight back to `read`; bash fills `COMPREPLY` **in-process** (completion functions need no readline context); zsh captures from a persistent `zsh/zpty` completion child driven by a real completion widget (see [04a §"Bash sidecar" / "Zsh sidecar"](04a-completion.md)). None `eval`s the user's line, and none emits `preexec` / `command_finished` / `precmd` — so a completion request runs no command and produces no block. `filenames` carries bash's `-o filenames` flag — `true` when the reply is a filename completion (matches are shell-escaped on accept), `false` when it is not (matches inserted **verbatim**, mirroring how bash itself only escapes glob/space chars for filename completions; see [04a §"Quoting and escaping"](04a-completion.md)). `cur` carries bash's `COMP_WORDBREAKS`-delimited current word — the word the completion function actually completed (`d FOO=ba` ⇒ `ba`) — so Termica re-bases the accept replace range onto it, matching bash, which replaces only that word ([04a §"Quoting and escaping"](04a-completion.md)). fish/zsh omit both fields (absent `filenames` ⇒ `false`, absent `cur` ⇒ no rebase); Termica treats every fish/zsh reply as filename-style and whole-token regardless, so only bash both emits and consumes them.
...
**Incomplete commands — the fish `PS2` analog.** Because the loop `eval`s each decoded command as a unit, an *incomplete* command (`echo 1 &&` on its own, an unterminated quote) would simply error. To match bash/zsh's continuation behavior, the loop first parse-checks the decoded command with `fish -n` (no-execute): an EOF-while-expecting-more error means "incomplete" → it emits a `continuation` marker and reads again WITHOUT executing (no preexec / command_finished / precmd — inert to the block model); a genuine syntax error executes so fish surfaces it. The full restore / whole-buffer-resend behavior is normative in [spec/04 §"Multi-line continuation"](04-prompt-editor.md#multi-line-continuation-ps2-marker).
