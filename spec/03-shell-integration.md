...
**Reply (shell → Termica):** `{"type":"completion","id":<n>,"filenames":<bool>,"value":[<line>, …]}` — a JSON array of the shell's **raw** completion lines (fish: `complete -C` output, `value\tdescription` or a space-padded table; zsh/bash: value-only lines). The split into value/description is done once, in Rust ([`parse_shell_complete`](../src/completion/drivers/parse.rs)), so the shells can't diverge. fish runs `complete -C <line>` **in-process** and loops straight back to `read`; bash fills `COMPREPLY` **in-process** (completion functions need no readline context); zsh captures from a persistent `zsh/zpty` completion child driven by a real completion widget (see [04a §"Bash sidecar" / "Zsh sidecar"](04a-completion.md)). None `eval`s the user's line, and none emits `preexec` / `command_finished` / `precmd` — so a completion request runs no command and produces no block. `filenames` carries bash's `-o filenames` flag — `true` when the reply is a filename completion (matches are shell-escaped on accept), `false` when it is not (matches inserted **verbatim**, mirroring how bash itself only escapes glob/space chars for filename completions; see [04a §"Quoting and escaping"](04a-completion.md)). fish/zsh omit the field (absent ⇒ `false`); Termica treats every fish/zsh reply as filename-style regardless, so only bash both emits and consumes it.
