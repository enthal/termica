...
- For a **path-shaped token**, `resolve_driver` additionally **drops two kinds of bad sidecar path candidate**, because the local path source is authoritative for on-disk paths:
  - **Non-extending** values. A real completion of a path token must extend it (case-insensitively). zsh's `cd` (directories-only) completion, when no directory matches the typed partial segment, falls back to emitting the **ancestor path components** — `cd /usr/bin/af⇥` captures `usr` and `bin`, which align to `/usr/bin/usr`, `/usr/bin/bin` (nonsense). They don't extend `/usr/bin/af`, so they're culled.
  - **Redundant** values. The local source already lists each directory **with its trailing `/`** (`~/Library/`); a slash-less sidecar twin (`~/Library`) is dropped so the local dir row wins — collapsing the double-row and ensuring that accepting a directory ends in `/` (keep completing into the path), never a space. Non-path tokens (command names, subcommands, flags — no `/`) skip both filters: fuzzy/substring command completions must survive, and a command on both `$PATH` and the sidecar still collapses via the ranker.
