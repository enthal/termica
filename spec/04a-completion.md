...
- Every surviving driver/sidecar value is **escaped for the shell context** (the cursor's quote state, [`escape_for_context`](../src/completion/local.rs)) exactly as the local path source escapes its own values — the sidecar returns raw names, so a completion with a space (`Application Support`) must become `Application\ Support` to round-trip. This also makes the redundancy check and the ranker dedup compare like-for-like against the already-escaped local values (otherwise `Application Support` and `Application\ Support` look distinct and both rows show). Escaping applies even for a non-path token, so an argument-position filename with a space (`vim my\ fi⇥`) round-trips too.
