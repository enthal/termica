...
| Ctrl + C (in the editor) | **No-op at an idle prompt** — the shell is idle at a confirmed prompt, so there is nothing to interrupt and a SIGINT would only print a cosmetic `^C`. Editor keeps its text; nothing reaches the PTY. (Interrupting a *running* program happens in `RawTerminal`, where the editor is inactive and Ctrl+C → SIGINT passes through.) **Exception:** while a PS2 continuation is pending (see [§Recovering from a stuck continuation](#recovering-from-a-stuck-continuation)), the shell is mid-line-read — Ctrl+C sends `\x03` to abort the dangling line and clears the editor. | same |
...
#### Recovering from a stuck continuation

Not every continuation is a *wanted* multi-line edit. Some commands the shell deems incomplete are surprises the user never intended — most notoriously `echo "!"` in zsh, where history expansion eats the closing quote and leaves an **unmatched `"`**, so the shell sits at `PS2` (`dquote>`) waiting for the quote to close. (Unclosed quotes generally, a stray trailing `&&`/`|`, and open `for`/`if`/`{`/here-docs do the same.) Because the continuation-restore hands back a normal-looking editor, the user can't *see* that the shell is stuck mid-line-read — and the shell has **already irrevocably received** `last_submitted + "\n"`, so nothing typed as a *command* can un-send it. Without an escape the pane is **locked**: every later submit just feeds the same dangling line and re-fires `PS2`.

Two recovery gestures, both of which abort the dangling line by sending the shell `\x03` (SIGINT) — exactly what Ctrl+C at a `PS2` prompt does in any terminal:

- **Ctrl+C** — the "give up entirely" gesture. Aborts the dangling line **and clears the editor**, returning to a fresh empty prompt. This is the *one* case where Ctrl+C is **not** inert in the editor (see the keybinding table above): a pending continuation means the shell is mid-line-read, **not** idle at a confirmed prompt, so the usual "nothing to interrupt" reasoning does not hold. ([`PaneSession::abort_continuation`](../src/pane.rs).)
- **Retype + Enter (2-Enter heal)** — the "run this instead" gesture. If the user clears the line and submits text that no longer **extends** the half-sent command (i.e. the new text does *not* start with `last_submitted`), `submit_editor_command` recognises the abandonment: instead of feeding the dangling line, it aborts it and resets the continuation state but **keeps the retyped text in the editor**. The shell returns to a fresh prompt; the next Enter — now a clean first submit — runs the retyped command. (An empty editor counts as abandonment, so "clear + Enter once" also escapes.) The legitimate multi-line case is unaffected: there the user *appended*, so the text still starts with `last_submitted` and the normal suffix-only diff-submit applies.

Both gestures funnel through one primitive ([`PaneSession::abort_continuation_line`](../src/pane.rs)) that resets `last_submitted` **before** the write, so the pane is never observed mid-abort with a stale prefix. Neither demotes: the shell stays at a prompt and the pane stays in `ShellPromptEditor`.

**Both gestures are gated to zsh / bash** (the shells that buffer a dangling line at the tty). **fish** never reaches this stuck state: it holds no partial command — its read-eval loop re-`eval`s the **whole cumulative buffer** each submit (see [§Multi-line continuation — base64 framing](03-shell-integration.md#L304)) — so a fish continuation recovers by simply retyping a complete command, which is re-evaluated fresh. There is no dangling line to SIGINT, so for fish the abandonment check ([`submit_abandons_continuation`](../src/pane.rs)) returns false (the retype falls through to the normal whole-buffer resend) and Ctrl+C stays inert.

The abort writes **`\x03\r`**, not a bare `\x03`. The zsh integration runs with `unsetopt zle` (cooked-mode tty — [`integration/zsh-bootstrap.zsh`](../integration/zsh-bootstrap.zsh)), and there a lone `\x03` aborts the dangling line but leaves the reader in a state that **swallows the very next command** — it arrives with no `preexec`, the shell reports the aborted line's `130` exit on the *following* prompt, and the user's command silently vanishes. The trailing `\r` completes the prompt cycle so the shell emits its `precmd` and the next command runs normally. (Verified empirically against a real zsh + the bootstrap: `\x03` alone, `\r\x03`, and double-`\x03` all eat the next command; `\x03\r` does not.)

The abort also primes the [`EchoSuppressor`](../src/echo_suppress.rs) to swallow the kernel's `^C\r\n` echo of the interrupt char. Unlike a real command-interrupt, the abort runs no command, so no `Running` block ever seals to absorb that `^C` — left alone it bleeds into the *next* command's block (a stray `^C` between the command and its output). Suppressing it is correct because the recovery is a Termica UI gesture, not a keystroke the user typed at the shell. The suppressor is prefix-match-and-disengage, so a terminal without `ECHOCTL` (no `^C` echo) simply passes through unchanged. Strict-layer tests live in [`src/pane.rs`](../src/pane.rs) (`ctrl_c_aborts_pending_continuation_and_unlocks_the_pane`, `retyping_a_different_command_during_continuation_heals_the_pane`) and [`src/render_pane.rs`](../src/render_pane.rs) (`ctrl_c_during_continuation_routes_to_abort`).
...
- **Never** sends a keystroke to the PTY while in `ShellPromptEditor` mode, except via `submit()`, Ctrl+D (EOF) on an **empty** editor (to exit an idle shell), and a Ctrl+C that aborts a **pending PS2 continuation** (see [§Recovering from a stuck continuation](#recovering-from-a-stuck-continuation)). The empty-editor EOF edge is enforced structurally at the input boundary by a single gate (`pty_passthrough_allowed`), not by a per-chord allow-list: every other keystroke — including a Ctrl+C at an *idle* prompt and unrecognised control chords like Ctrl+X/Y/Z — is swallowed. A leaked raw control byte sits in the shell's input and resurfaces prefixed to the next command's output (`^X^Y^Z…`), and a leaked `\x03` (Ctrl+C) **at an idle prompt** aborts the shell's empty line so the submitted command vanishes. That is why Ctrl+C is inert *at an idle prompt*: there is no program to interrupt (that happens in `RawTerminal`), so a SIGINT would only print a cosmetic `^C`. The continuation case is the deliberate exception — there the shell *is* mid-line-read and `\x03` is exactly the right byte (it routes through `abort_continuation`, ahead of the gate, not through it). The earlier per-letter consume list only covered a handful of chords and leaked the rest of the alphabet; the boundary gate makes the idle-prompt leak unrepresentable. (Strict layer — tested.)
...
**Focused-editor chrome.** When the prompt-editor caret would be drawn ([§"When is the caret shown?"](#when-is-the-caret-shown)), a rounded border is drawn around BOTH the chip bar AND the editor body together — the default shipped variant is a soft outer glow (`ChromeVariant::I` in [`src/focused_chrome.rs`](../src/focused_chrome.rs); the live `--pick-chrome` viewport can swap variants). The border says "this whole prompt surface is wired for input"; same predicate as the caret, and it fades in/out over ~0.5 s with the caret.

The command area always carries an **opaque terminal-background backing** (the rounded card), painted below the chip + editor and above the scrollback — present whether or not the pane is focused, so scrollback never crowds the chips and the prompt reads as a solid card on every pane; only the border fades with focus. The backing reaches the border's inner edge on the left / right / bottom (so no scrollback shows in the band between the editor content and the border) and keeps a little extra opaque headroom above the chips. Backing and border derive their geometry from one source (`focused_chrome::backing_shape` / `backing_rect`) so they cannot drift apart. The footer content (chip row + command) is clipped to the backing's right edge, so a long path / branch / command clips flush at the border instead of spilling past it. (Visual tuning is open: see the `pick_*` follow-ups.)
