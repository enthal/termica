...
`window.layout_blob` is a **JSON-serialized `SavedLayout`** ([`src/persist/layout.rs`](../src/persist/layout.rs)): the `egui_tiles::Tree<PaneId>` topology (tabs, splits, active leaf — egui_tiles' own serde) plus a `db_pane_by_app` map from each app `PaneId` to its durable `pane` row id, so a restored leaf can find its scrollback chunks (chunks are keyed by the db pane id, which outlives any single process). JSON over bincode: a layout blob is tiny and human-inspectable, and reuses the `serde_json` we already depend on. The format is owned by Termica; we ship a migration when it changes.

**Window geometry.** The blob also carries an optional `window_geometry` (inner size + outer position, in egui logical points) so a relaunch reopens the OS window where it was. It is sampled each frame and folded into the same change-fingerprint + debounced write as the topology, so a resize/move is persisted ~1 s after it settles. The field is `Option` with `#[serde(default)]`: a pre-geometry blob deserializes to `None` (no migration) and the window opens at its default size. On launch the geometry is applied by the `ViewportBuilder` (before the monitor size is known) and then **clamped to the current monitor on the first frame** (`WindowGeometry::clamp_to_monitor`) — a workspace saved on a large display reopens *usably* on a smaller one rather than oversized or off-screen. Because we persist geometry ourselves (incrementally, kill-surviving), eframe's own `persist_window` is disabled so the two don't fight.

**Written on change, not on quit.** The blob is re-written (`save_layout`, single-window app → clear-and-insert one workspace + window) whenever the tile tree changes **structurally** — a new/closed tab, a split, a drag-reorder, an active-tab switch — debounced (~1 s) so a burst of edits coalesces into one write. A clean-quit flush remains as a backstop, but it is *only* a backstop: the process can vanish (crash, `SIGKILL`, hard reset) with no teardown, so an on-quit-only write would lose the entire workspace — exactly the "no silent data loss" failure the rest of this document is built to avoid. The debounced write is small (a few-KB blob, ≤ 1/s) and stays on the UI thread for the same reason the cwd write is cheap: it is user-paced and coalesced.

**The saved blob is self-consistent.** Only leaves that map to a durable `pane` row in `db_pane_by_app` are serialized; a live pane still in a degraded / not-yet-persisted state is pruned from the tree *before* it is written (the prune collapses now-empty containers, the same operation restore uses). So the writer never emits a blob whose tree references a leaf the map can't resolve — restore is never handed an unmapped leaf of our own making.
...
1. Read the most recent `workspace` row whose sessions are **not owned by a live process** (see §"Concurrent processes and session ownership" — a workspace still held by another running Termica is left alone, never adopted). **This liveness gate is intentionally workspace-level and conservative:** if *any* mapped pane's latest session lock is currently held, the whole workspace belongs to a running process and is declined — Termica never partially adopts a workspace another process is using. This safety rule is distinct from the leaf-level *resilience* in step 3 (which concerns gone/corrupt data, never live ownership).
2. For each window: deserialize `layout_blob`, restore tile tree.
3. **Restore is resilient, not all-or-nothing.** For each leaf: a leaf that maps to a durable `pane` row restores; a leaf with no mapping (only possible from an older or externally-mangled blob — our own writer never emits one, see §"The saved blob is self-consistent") is **pruned** from the tile tree (`Tree::remove_recursively` + `simplify`, then a Tabs-`active` revalidation pass), and the *rest* still restore. Restore falls through to a fresh single pane **only when nothing is restorable** (every leaf unmapped, or the tree simplified to empty) — one bad leaf must never cost the user their whole workspace. For each restorable pane: create a `Pane` in `Dead` mode, attach its transcript view to its persisted chunks. A pane whose chunks are **all missing or corrupt** still restores — as an **empty** `Dead` pane carrying its persisted cwd / title — rather than aborting; a row pointing at a gone chunk degrades to a gap, never a dropped pane. Each restored block's command label / exit / cwd / git header is rebuilt from the chunk's `chunk_chip` rows (§"Schema v4"). Chunks are logical lines, so they re-wrap to the *current* (restore-time) pane width — a workspace saved on a wide monitor restores cleanly onto a narrow one. (There is no clear-watermark to honour: Cmd-K deletes the cleared chunks outright, so anything that survives to restore was never cleared.)
