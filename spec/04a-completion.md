...
>
> On a request, `__termica_bc_capture <line>` (in [`integration/bash-bootstrap.bash`](../integration/bash-bootstrap.bash)) splits the line into `COMP_WORDS` the way **readline** does — on whitespace AND on each `$COMP_WORDBREAKS` char (`=`, `:`, `@`, …), each break char its own token — so `export FOO=ba`<Tab> and `scp host:pa`<Tab> hand the function the right `cur` (`ba` / `pa`, not `FOO=ba` / `host:pa`), and bash-completion's own `_get_comp_words_by_ref -n :` can re-merge from it. (Quote / backslash handling is a later refinement; the unquoted common case is covered.) It then sets the rest of the `COMP_*` globals, triggers bash-completion's lazy loader (`_comp_load`) for the command, reads the registered `-F <function>` from `complete -p` (or the dynamic `-D` default), calls that function, and dedupes `COMPREPLY` into a values array. The `COMP_*` are all `local`, so they can never leak into the next real command's bash-preexec DEBUG trap (which **skips** preexec while `COMP_POINT` is set — a stuck value would silently stop the block model).
>
> The capture also records bash's **`-o filenames`** state for the reply ([#178](https://github.com/enthal/termica/issues/178)), so the accept path knows whether to shell-escape the matches (see [§"Quoting and escaping"](#quoting-and-escaping)). It can be set two ways, both observed: **statically** in the spec (`complete -o filenames -F …`, read straight from `complete -p`), or **dynamically** by the function (`compopt -o filenames`, what bash-completion's `_filedir` calls). The dynamic form is invisible to `complete -p`, so `__termica_bc_capture` shims `compopt` for the duration of the function call — recording `-o`/`+o filenames` while forwarding faithfully to the builtin — then restores it. The flag rides out in the marker's `filenames` field. The capture likewise records the `COMP_WORDBREAKS` current word (`COMP_WORDS[COMP_CWORD]`) and emits it as the marker's `cur` field, so Termica re-bases the accept replace range onto exactly the word bash completed (see [§"Quoting and escaping"](#quoting-and-escaping), [#183](https://github.com/enthal/termica/issues/183)); it is recorded before the early-return paths so a no-completion command still reports its own word (`cur == token`, no rebase).
>
...
- **The bash accept replace range follows `COMP_WORDBREAKS`** ([#183](https://github.com/enthal/termica/issues/183)). Termica's editor token splits on whitespace/quotes only, but bash splits `COMP_WORDS` on `$COMP_WORDBREAKS` too (`=`, `:`, …) and a `complete -F` function completes — and bash replaces — only that narrower word. So `d FOO=ba⇥` returns a value for `ba`, not `FOO=ba`; replacing the whole token would yield `d <value>` instead of `d FOO=<value>` (and `scp host:/pa⇥` would drop the `host:` prefix). The bash sidecar reports its `cur` (the `COMP_WORDBREAKS` current word) in the reply; [`rebase_replace_to_word`](../src/completion/mod.rs) moves the popup's `origin_byte` to where `cur` begins (`cur` is a suffix of the token). Accept replaces `[origin_byte, cursor)`, so re-basing the origin is all it takes; the carried locals (gathered for the wider token) are dropped when a word-break trims the range. `/`-only paths are unaffected (`/` is not a `COMP_WORDBREAKS` char, so `cur == token`) — the segment realign above still owns them. fish/zsh report no `cur` and keep the whole-token range.
...
- Every surviving driver/sidecar value is **canonicalised then escaped for the shell context** ([`resolve_driver`](../src/completion/mod.rs)), exactly matching how the local path source escapes its own values — this matters for the no-local-listing path case and for non-path argument filenames:
  ...
  - **…but only for filename completions** ([#178](https://github.com/enthal/termica/issues/178)). bash itself only escapes glob/space metacharacters when the completion is a **filename** completion (`-o filenames`); a plain `complete -F` reply that is not filename-oriented (`export FOO=ba⇥` → `cur=[ba]`, a branch name, a flag value) is inserted **verbatim**. The bash sidecar reports this per reply (the `filenames` flag — see [§"Bash sidecar"](#bash-sidecar) and [spec/03 §completion](03-shell-integration.md)), and [`resolve_driver`](../src/completion/mod.rs) skips the escape when it is `false`, so `cur=[ba]` stays `cur=[ba]` rather than becoming `cur=\[ba\]`. fish/zsh sidecars and the CLI drivers always escape (their captured values are filename-style and their space-handling depends on it).
