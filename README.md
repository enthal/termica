...
> **🚧 Status: WIP — Phase 2 (Workspace) in flight.** Phase 1 is complete: a single PTY-backed terminal pane runs `vim`, `less`, `htop`, `fzf`, `ssh` interactively, with mouse selection / clipboard, clickable URLs and on-disk paths, and a VT golden suite locking in escape-sequence parsing. Phase 2A has landed: an `egui_tiles::Tree<PaneId>` workspace with tabs and drag-splits; per-tab keyboard focus; Cmd/Ctrl+Shift shortcuts for new / close / next / prev tab and Quit; modal confirmation when closing a tab or quitting with a program still running; a custom macOS menubar that routes Quit through the same modal. Remaining Phase 2 items (spawn-in-cwd, duplicate-here, split snapshot tests) are open follow-ups; after Phase 2: markers + mode machine ([#3](https://github.com/enthal/termica/issues/3)). Watch this banner — it tracks the actual phase as work lands.
