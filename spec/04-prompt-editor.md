...
**Focused-editor chrome.** When the prompt-editor caret would be drawn ([§"When is the caret shown?"](#when-is-the-caret-shown)), a rounded border is drawn around BOTH the chip bar AND the editor body together — the default shipped variant is a soft outer glow (`ChromeVariant::I` in [`src/focused_chrome.rs`](../src/focused_chrome.rs); the live `--pick-chrome` viewport can swap variants). The border says "this whole prompt surface is wired for input"; same predicate as the caret, and it fades in/out over ~0.5 s with the caret. The bordered region is filled with an **opaque terminal-background backing** that reaches exactly the border's inner edge, painted below the chip + editor and above the scrollback, so no scrollback shows through the band between the editor content and the border. Backing and border derive their geometry from one source (`focused_chrome::backing_shape`) so they cannot drift apart. (Visual tuning is open: see the `pick_*` follow-ups.)
