...
- For a **path-shaped token** — i.e. when the completion is a **path extension** — `resolve_driver` additionally **must not extend to a bad path**. (Not every completion is a path extension: command names, subcommands, branches, and flags pass through untouched.) It drops three kinds of bad sidecar path candidate, because the local path source is authoritative for on-disk paths:
  - **Non-extending** values. A real completion of a path token must extend it (case-insensitively). When no entry matches the typed partial segment, zsh can emit unrelated words; aligned, those don't extend the token, so they're culled.
  - **Ancestor-component** values. zsh prepends the typed path's own directory components to the match set: `cd /usr/⇥` captures `usr`, and `cd /usr/bin/af⇥` captures `usr` and `bin` (verified against the live captive child) — alongside the real children. Aligned, those leaves equal a component the token already names (`/usr/usr`, `/usr/bin/bin`) — never a real child — so any candidate whose leaf is one of the token's own directory components is dropped. (The extend check alone misses this for a trailing-slash token, where the empty partial segment makes the prefix test vacuous.)
  - **Redundant** values. The local source already lists each directory **with its trailing `/`** (`~/Library/`); a slash-less sidecar twin (`~/Library`) is dropped so the local dir row wins — collapsing the double-row and ensuring that accepting a directory ends in `/` (keep completing into the path), never a space.

  Non-path tokens (no `/`) skip all three filters: fuzzy/substring command completions must survive, and a command on both `$PATH` and the sidecar still collapses via the ranker.
- Every driver/sidecar value is **canonicalised then escaped for the shell context**, exactly matching how the local path source escapes its own values:
  - **Canonicalise** — unescape the value to its literal ([`unescape`](../src/completion/local.rs)). zsh's `compadd` capture emits the SAME match in BOTH forms (verified live): raw `Application Support` *and* pre-escaped `Application\ Support`. Unescaping both to the literal lets them collapse to one candidate; without it the pre-escaped form would double-escape to `Application\\\ Support` and survive as a junk row.
  - **Escape** — re-escape the literal for the cursor's quote state ([`escape_for_context`](../src/completion/local.rs)), so a completion with a space round-trips (`Application Support` → `Application\ Support`) and the redundancy check / ranker dedup compare like-for-like against the already-escaped local values (otherwise raw and escaped look distinct and both rows show). Escaping applies even for a non-path token, so an argument-position filename with a space (`vim my\ fi⇥`) round-trips too.
