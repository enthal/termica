...
Both gestures funnel through one primitive ([`PaneSession::abort_continuation_line`](../src/pane.rs)) that resets `last_submitted` **before** the write, so the pane is never observed mid-abort with a stale prefix. Neither demotes: the shell stays at a prompt and the pane stays in `ShellPromptEditor`. Strict-layer tests live in [`src/pane.rs`](../src/pane.rs) (`ctrl_c_aborts_pending_continuation_and_unlocks_the_pane`, `retyping_a_different_command_during_continuation_heals_the_pane`) and [`src/render_pane.rs`](../src/render_pane.rs) (`ctrl_c_during_continuation_routes_to_abort`).
