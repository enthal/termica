...
site/
  ...
  infra/
    ...
    deploy.sh            # repeatable publish: s3 sync + cache invalidation
...
The golden rule: **`src/` is the deploy artifact.** Anything in `src/` is
uploaded as-is. Sources that shouldn't ship (raw screenshots) live outside it,
in `media-src/`.
...
`index.html` uploads with `no-cache`; other assets get a 7-day cache. Every
deploy issues a `/*` CloudFront invalidation, so changes appear within ~a minute.
