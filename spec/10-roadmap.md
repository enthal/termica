...
- ⏳ **4G — Block header chrome.** Split into:
    ...
    - ⏳ **4G-async-context — git branch + dirty chips.** Async-probe surface per spec/00 §"Do not block the UI on probes"; co-locates with Phase 5's `termica-context` work since the same probes power both surfaces. Sliced:
        ...
        - ✅ **PR 4 — GitHub PR chip.** New [`src/pr_context.rs`](../src/pr_context.rs) (`parse_pr_view` → `PrContext { number, ci }`, CI roll-up over `statusCheckRollup`; pure, via `serde_json::Value`) + [`src/gh_probe.rs`](../src/gh_probe.rs) (per-pane `GhProbe` running `gh pr view` off-thread, re-probed on cwd / branch change, **self-re-polling while CI is pending** so a watched run refreshes itself, cancelled on teardown). `render::paint_block_header` renders a `PR #NN` chip after the git chips, colored by CI (green / yellow / red), **on the live prompt header only** (a finished command's CI is meaningless on scroll-back). Parser + roll-up + probe round-trip unit-tested; two PR-chip snapshots.
        - ✅ **PR 5 — chip visual polish** ([#130](https://github.com/enthal/termica/pull/130)). Green branch chip (`BLOCK_HEADER_BRANCH_FG`); sealed (historical) blocks render every chip muted (`fade_chip_color`, ~62% toward grey, threaded via a `faded` arg on `paint_block_header`) so finished blocks read past-tense while live prompt / running chips stay vivid.
        - ✅ **PR 6 — periodic context refresh.** Both probes now **self-poll** so the chips track changes made outside this pane: `git_probe` re-runs every 5s while the cwd is a repo (cheap, no network — catches edits / `git checkout` done by other tools); `gh_probe` re-runs adaptively while a PR is open (20s pending CI, 90s settled, none without a PR — dozens of `gh` calls/hr/pane, well inside GitHub's 5000/hr). Cadence policy (`self_poll_interval`) is unit-tested. Remaining 4G follow-ups: more visual tweaking, the safety chips (kube / aws / ssh).
...
- ✅ **9D — Writer + seal pipeline**: WAL + `busy_timeout` (9D-i); `Persistence::begin_session` allocating the `pane` + `session` rows and the on-disk `scrollback/session-<id>/pane-<id>/` directory (9D-i); per-pane background writer thread that persists **one chunk per sealed block** — encode + zstd + atomic temp-then-rename + `scrollback_chunk` row, with a cumulative logical-line cursor (9D-ii). *Deferred to future hardening* (see spec/08 §Consistency): the live `current.chunk` append of stabilized rows that would bound mid-command crash loss to ~1s; and an end-to-end real-shell integration test.
...
- **Tab completion engine** (CLI-native drivers + per-pane shell sidecar for bash / zsh / fish) per [04a](04a-completion.md). Supersedes the earlier "OSC request/response shell-completion bridge" stub — the same goal, cleaner mechanism (private stdio JSON sidecar, not an OSC channel over the PTY). Implementation slices: drivers → fish sidecar → bash sidecar → zsh sidecar, in that order.
  ...
  - ✅ **Bash live-shell completion** (shipped). A bash pane at a prompt answers **in-process** — bash completion functions just fill `COMPREPLY` and need no readline context, so (unlike zsh's captive `zpty` child) there's no sidecar process: `__termica_bc_capture` sets `COMP_*`, triggers bash-completion's lazy loader, calls the registered `-F` function, and dedupes the result. The bootstrap loads bash-completion if `~/.bashrc` didn't. Dispatched as the shared guarded `__termica_complete` command, but with **no leading space** (bash-preexec reads from `history 1`, so a leading-space/`ignorespace` sentinel would corrupt the block model) — the function deletes its own history entry instead. Cobra drivers stay authoritative; bash handles the long tail; values only in v1. Graceful degradation on old bash (< 4.1 / no bash-completion): capability reported in `integration_ready`. `DriverTool::BashComplete`. ([04a §"Bash sidecar"](04a-completion.md#bash-sidecar)).
  ...
  - Remaining: a one-time UI notice when `integration_ready.completion` is false (old bash); bash/zsh PS2-style continuation parity for fish.
